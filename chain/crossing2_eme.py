"""The crossing alone, second run: identical to crossing_eme.py except that the mode solver has no
absorbing layers, after the first run dropped the light at one cell boundary near 216 nm.
Writes chain/eme/crossing2_te3d.json. Originally: the die's linear taper from 180 to 240 nm (375 um at 0.16 nm/um), 30 cells, 16 modes,
...
G = BR.G
NUM_MODES = 16
# no absorbing layers in the mode solver: orthogonal modes, exact expansion at every cell boundary   # was 8: with 8 modes the first run lost 25% of an untouched glass mode to basis truncation across 120 cell interfaces
...
sim = td.EMESimulation(
    ...
    eme_grid_spec=td.EMEUniformGrid(num_cells=30, mode_spec=td.EMEModeSpec(num_modes=NUM_MODES, num_pml=(0, 0))),
