...
for y, lab in ((0, "in phase"), (90, "quarter cycle behind"), (180, "half cycle behind")):
    ...
    ax[0].text(1.98, y + 4, lab, ha="right", fontsize=9.5, color="#5c6b7a")
ax[0].axvline(1.0, color="#c0392b", lw=1.2, ls="--")
ax[0].text(1.04, 30, "resonance ω₀", color="#c0392b", fontsize=10)
ax[0].text(0.10, 152, "transparent window\nwhere glass is used", fontsize=10, color="#2a6b3a")
ax[0].set_xlabel("driving frequency  ω / ω₀", fontsize=10.5)
ax[0].set_ylabel("lag of the electron behind the force  (degrees)", fontsize=10.5)
ax[0].set_title("How far the electron lags", fontsize=12)
...
ax[1].set_ylabel("n′", fontsize=10.5, color="#1b6ca8")
ax[1].set_xlabel("driving frequency  ω / ω₀", fontsize=10.5)
ax2 = ax[1].twinx()
ax2.plot(w, n.imag, lw=2.2, color="#c0392b", ls="--", label="absorption  n″")
ax2.set_ylabel("n″  (absorption)", fontsize=10.5, color="#c0392b")
ax[1].axvline(1.0, color="#c0392b", lw=1.2, ls="--")
ax[1].text(0.08, 1.075, "n rises with frequency here:\nnormal dispersion", fontsize=9.5, color="#2a6b3a")
ax[1].set_title("What that lag produces", fontsize=12)
...
fig.suptitle("One driven electron explains the refractive index, dispersion and absorption",
